...
original_author="SamuelWitke"
original_name="mars_sre_runbook_api"
original_urlname="mars_sre_runbook_api"
original_description="Awesome mars_sre_runbook_api created by SamuelWitke" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/mars_sre_runbook_api "${name}"
...
then
    ...
    echo "then run 'mars_sre_runbook_api' or 'python -m mars_sre_runbook_api'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'mars_sre_runbook_api' or 'python -m mars_sre_runbook_api' or 'poetry run mars_sre_runbook_api'"
fi
