...
- 🎯 Entry points to execute your program using `python -m <mars_sre_runbook_api>` or `$ mars_sre_runbook_api` with basic CLI argument parsing.
...
---
# mars_sre_runbook_api Flask Application

Awesome mars_sre_runbook_api created by SamuelWitke
...
```bash
git clone https://github.com/SamuelWitke/mars_sre_runbook_api mars_sre_runbook_api
cd mars_sre_runbook_api
make install
```
...
```bash
pip install mars_sre_runbook_api
```
...
```bash
$ mars_sre_runbook_api
```

or

```bash
$ python -m mars_sre_runbook_api
```
...
```bash
mars_sre_runbook_api create-db   # run once
mars_sre_runbook_api populate-db  # run once (optional)
mars_sre_runbook_api add-user -u admin -p 1234  # ads a user
mars_sre_runbook_api run
```
